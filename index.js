...
async function run() {
    try {
        ...
        const productListingsBySellers = client.db("AutoFusion").collection("oldCarsByUsers");
        ...
        app.post("/newCarSellByUser", isToken, async (req, res) => {
            try {
                const result = await productListingsBySellers.insertOne(req.body);
                res.send(result);
            } catch (error) {
                console.error("Error:", error);
                res.status(500).send({ message: "Internal Server Error." });
        ...
        app.get("/allFeedbacks", async (req, res) => {
            try {
                const result = await feedbackCollection.find().sort({ _id: -1 }).limit(5).toArray()
                res.send(result);
            } catch (error) {
                console.error("Error:", error);
                res.status(500).send({ message: "Internal Server Error." });
        ...
        app.get("/homeListings", async (req, res) => {
            try {
                const result = await productListingsBySellers.find().sort({ _id: -1 }).limit(8).toArray();
                res.send(result);
            } catch (error) {
        ...
        app.get("/topBidHomeListings", async (req, res) => {
            try {
                const result = await productListingsBySellers.find().sort({ totalBids: -1 }).limit(8).toArray();
                res.send(result);
            } catch (error) {
        ...
        app.put("/updateSellerVerification/:id", isToken, isAdmin, async (req, res) => {
            try {
